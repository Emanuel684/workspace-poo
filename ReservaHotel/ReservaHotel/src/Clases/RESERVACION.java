...
public class RESERVACION {
			...
			if (INICIO.datosClientes[i][0].equals(cedulaCliente)) {
				...
				for (int j = 0; j < INICIO.datosReservas.length; j++) {
					...
					if (INICIO.datosReservas[j][4].equals("")) {
						...
						INICIO.datosReservas[j][4] = precioHabitacion.ReservarHabitacionPrecio(numeroHabitacion,
								cantidadDias, cedulaCliente, INICIO.datosReservas[j][5]);
		...
		for (int i = 0; i < INICIO.datosClientes.length; i++) {
			...
			if (INICIO.datosClientes[i][0].equals(cedulaCliente)) {
				...
				for (int j = 0; j < INICIO.datosReservas.length; j++) {
					...
					if (INICIO.datosReservas[j][3].equals(numeroHabitacion)
							&& INICIO.datosReservas[j][2].equals(cedulaCliente)) {
						...
						INICIO.datosReservas[i][4] = precioHabitacion.ReservarHabitacionPrecio(numeroHabitacion,
								cantidadDias, cedulaCliente, INICIO.datosReservas[j][5]);
	...
	public void PagarReserva(String numeroReserva, String cedulaCliente) {
		for (int i = 0; i < INICIO.datosClientes.length; i++) {
	...
	public String EstimarReservacion(String tipoHabitacion, String cedulaCliente, String cantidadNoches) {
		...
		if (tipoHabitacion == "Habitaciones sencilla") {
			...
			float cantidadDescuentoTotal = (Integer.parseInt(SIMPLE.Precio) * Integer.parseInt(cantidadNoches)
					* Integer.parseInt(descuento.split("%")[0])) / 100;

			precioTotal = String.valueOf(
					(Integer.parseInt(SIMPLE.Precio) * Integer.parseInt(cantidadNoches)) - cantidadDescuentoTotal);

		} else if (tipoHabitacion == "Habitaciones doble") {

			float cantidadDescuentoTotal = (Integer.parseInt(DOBLE.Precio) * Integer.parseInt(cantidadNoches)
					* Integer.parseInt(descuento.split("%")[0])) / 100;

			precioTotal = String.valueOf(
					(Integer.parseInt(DOBLE.Precio) * Integer.parseInt(cantidadNoches)) - cantidadDescuentoTotal);

		} else {

			float cantidadDescuentoTotal = (Integer.parseInt(MATRIMONIAL.Precio) * Integer.parseInt(cantidadNoches)
					* Integer.parseInt(descuento.split("%")[0])) / 100;

			precioTotal = String.valueOf(
					(Integer.parseInt(MATRIMONIAL.Precio) * Integer.parseInt(cantidadNoches)) - cantidadDescuentoTotal);
